...
var span = document.querySelector(selectors = '#createdBy');
span.innerHTML = span.innerHTML + ' Martina';
...
function buildToDo(todo, index) {
  // Tu código acá:
  var toDoShell = document.createElement(tagName = 'div');
  toDoShell.className = 'toDoShell';

  var toDoText = document.createElement(tagName = 'span');
  ...
  toDoText.addEventListener(type = 'click', completeToDo);
...
function displayToDos() {
  // Tu código acá:
  let toDoContainer = document.querySelector(selectors = '#toDoContainer');
  toDoContainer.innerHTML = '';

  let buils = buildToDo(toDoItems);

  for (let i = 0; i < buils.length; i++) {
    toDoContainer.appendChild(buils[i])
  }
}
...
function addToDo() {
  // Tu código acá:
  let input = document.querySelector(selectors = '#toDoInput');
  if (input.value !== '') {
...
// Tu código acá:
let add = document.querySelector(selectors = '#addButton');
add.addEventListener(type = 'click', addToDo);
